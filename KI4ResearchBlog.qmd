...
Here is a condensed version of the prompt given to an AI assistant with access to the web and code execution (the full prompt is in the footnote[^1]):
...
1. **Enable extended reasoning** for complex questions so the model thinks step by step before answering, rather than pattern-matching to the most plausible response.
2. **Give the model specific, expert instructions** so it behaves like a domain specialist rather than a generalist assistant.
3. **Fill the context window with curated documents and domain knowledge** so the model reasons from evidence rather than from model-memory.
4. **Extend context retrieval dynamically** using semantic search over a larger document library (RAG).
...
Most major LLM interfaces now offer an extended reasoning mode — variously labelled *thinking*, *deep think*, *extended thinking*, or *deep research* depending on the provider. When enabled, the model allocates additional compute to step-by-step reasoning before generating its final response, rather than immediately producing the most statistically likely continuation.
...
It is a single toggle, costs nothing beyond the extra tokens used, and is the highest-leverage change you can make for analytical questions. Enable it by default for anything methodologically demanding; turn it off for simple drafting tasks where speed matters more.
...
A practical shortcut: paste a weak first draft of a prompt and ask the model to identify what is underspecified. It will often surface the gaps you missed.
